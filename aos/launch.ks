...
RUN common.ks.
RUN ui.ks.
RUN crash_monitor.ks.
RUN launch_stages.ks.
RUN orbital.ks.
...
// Start filling balloons now because it takes a while.
fillBalloons().
// RUN_MARQUEE(BUILD_ORDER(), 0.2, 6, FALSE).

// Animated intro sequence
INTRO_SEQUENCE_1().

AG_TOGGLE(AGX_LEGEND["legs"]).

LAUNCH_PRINT_WELCOME().

INTRO_SEQUENCE_2().
PRINT " ".
PRINT "Press any key to confirm you will die if you do that...".
// Why does TERMINAL:REVERSE not work at all?
// TERMINAL:REVERSE.
...
// Start crash monitoring system
START_CRASH_MONITOR().
LOG_INFO("Starting nuclear reactor...").
WAIT .2.
AG_TOGGLE(AGX_LEGEND["fan_system"]).
LOG_INFO("Powering up fans...").
// Reverse fans
AG_TOGGLE(AGX_LEGEND["reverse_fan"]).
WAIT 0.5.  // Reduced wait time from 1.5 seconds to 0.5 seconds

// Throttle up and release the brakes
LOG_STAGE("Launch stage: 1, Back up sequence.", "Ground Operations - Backing Up").
...
WAIT 0.5.  // Reduced wait time from 5 seconds to 0.5 seconds
lvlBalloon().
// RUN_MARQUEE(BUILD_ORDER(), 0.05, 4, true).
CLEARSCREEN.

// Driving part
LOG_STAGE("Launch stage: 2, Taxi to takeoff.", "Runway Operations - Taxiing to Takeoff").

// Disable intro sequences to prevent UI conflict with runway HUD
...
LOG_STAGE("Post-orbit: Opening up ship!", "Ship Systems Deployment").
//open hanger
WAIT 2.
...
WAIT 2.5.
AG_TOGGLE(AGX_LEGEND["lights_hab"]).
AG_TOGGLE(AGX_LEGEND["innerflood_lights"]).
WAIT 4.
...
AG_TOGGLE(AGX_LEGEND["drill_motor_locks"]).
WAIT 1.
//depoly boom
...
AG_TOGGLE(AGX_LEGEND["drill_motor_locks"]).
AG_TOGGLE(AGX_LEGEND["drill_motors"]).
WAIT 2.
//depoly comms
...
// come in retrograde and burn then turn around real quick
// to hit the atmo leaving 50 dv left for adjustments on the final pass
//---------------------Entry--------------------------------------//
// ag7 for pitch with airbrake. brake button toggles airbrakes but
// it negates pitch control so use when pitch control is not needed.
// When safe, right the plane, start fans, fly up to stall,
// eject heat shield. this is hard because the heat sheild always
// wants to hit you as soon as it is ejected. Why a total verticle
// stall works best.
//---------------------Find a place to land-----------------------//
// Need flat landing place maybe with resources. 
//----------------------Land--------------------------------------//
// Landing flaps and reverse thrust to get to landing speed. 
// when landing use forward fan thrust and pitch with airbrakes.
// on touchdown, reverse thrust and brake.
// shutdown fans and raise stabilizers. open up ship same as for space.
// open all hangers and bay doors. then take off fuel tanks.

// --------------------Make base--------------------------//
// get resources and make new bases and rockets.
// dont forget to survive and make food and water and air
// we can float around eve forever to find what is there.
// we can get small supply drops for missing resources.

//-------------------fly home (or to eve?)------------------//
// Have to build disposable engines that attach to the bottom
// That way we can just land on it and hook up. the drop later.
// need some crazy engines. solids? 